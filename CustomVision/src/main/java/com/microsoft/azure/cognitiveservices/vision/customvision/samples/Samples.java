...
public class Samples {
    ...
    private static void AddImageToProject(Trainings trainer, Project project, String fileName, byte[] contents, UUID tag, double[] regionValues)
    {
        ...
        if (regionValues != null)
        {
            ...
            file = file.withRegions(asList(region));
        } else {
            batch = batch.withTagIds(asList(tag));
        }
    ...
    {
        try {
           return ByteStreams.toByteArray(Samples.class.getResourceAsStream(folder + "/" + fileName));
        } catch (Exception e) {
            System.out.println(e.getMessage());
            e.printStackTrace();
        }
